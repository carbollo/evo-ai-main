...
class Settings(BaseSettings):
    ...
    # Database settings
    # Prefer DATABASE_URL (Railway/Heroku style), fallback to POSTGRES_CONNECTION_STRING
    DATABASE_URL: Optional[str] = os.getenv("DATABASE_URL")
    POSTGRES_CONNECTION_STRING: str = os.getenv(
        "POSTGRES_CONNECTION_STRING",
        DATABASE_URL or "postgresql://postgres:root@localhost:5432/evo_ai",
    )
...
def get_settings() -> Settings:
    ...
    # Normalize legacy postgres scheme if needed (e.g. postgres:// -> postgresql://)
    if settings_instance.POSTGRES_CONNECTION_STRING.startswith("postgres://"):
        settings_instance.POSTGRES_CONNECTION_STRING = (
            settings_instance.POSTGRES_CONNECTION_STRING.replace(
